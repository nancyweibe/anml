...
function init() {
  window.addEventListener(
    ...
    function(event) {
      ...
      link[link.length - 1]
        ? navigate(event.target.location.href, 0, 650, null)
...
function customCursor() {
  ...
  const updateProperties = (elem, state) => {
    elem.style.setProperty(
      "transform",
      `translate(${state.x - state.width / 2}px, ${state.y -
        state.height / 2}px)`
    );
  ...
  document.addEventListener("mousemove", e => {
    ...
    if (
      document.elementFromPoint(e.clientX, e.clientY).closest("[data-theme]")
    ) {
      let theme = document
        .elementFromPoint(e.clientX, e.clientY)
        .closest("[data-theme]").dataset.theme;
      cursor.classList.remove("black", "white");
      cursor.classList.add(theme);
      theme == "black"
        ? document.body.setAttribute(
            "style",
            "cursor:url('./assets/img/cursor-black.svg') 3 3, auto"
          )
        : document.body.setAttribute(
            "style",
            "cursor:url('./assets/img/cursor.svg') 3 3, auto"
  ...
  function render() {
    state = createState({
      clientX: lerp(state.x, mouseX, 0.15),
      clientY: lerp(state.y, mouseY, 0.15)
    });
...
function applyTemplate(calback) {
  ...
  let elements = document.querySelectorAll(
    ".hero, .content, .next, .mask-scroll, .cover-video, .overlay"
  );
  ...
  xmlhttp.onreadystatechange = function() {
    if (xmlhttp.status == 200 && xmlhttp.readyState == 4) {
      ...
      newLoaded.innerHTML = body.replace(
...
function initConfigs() {
  ...
  if (gridmobdisabled)
    gridmobdisabled = gridmobdisabled.dataset.gridmobdisabled;

  if (window.innerWidth < 991)
    if (gridmobdisabled) grid.classList.add(gridmobdisabled);
...
function initMaskScroll() {
  ...
  if (maskScroll || window.innerWidth > 991) {
    MaskScrollClass = new MaskScroll(maskScroll, {}, data => {
      data.dataset.grid
        ? addClassToGrid(data.dataset.grid)
...
function setHeaderTheme() {
  ...
  let theme = document.elementFromPoint(0, 50).closest("[data-theme]").dataset
    .theme;
...
function initCarousels() {
  ...
  black.forEach(gallery => {
    ...
    function onInit() {
      ...
      nav.innerHTML =
        "<button type='button'><i class='block-gallery-prev icon-arrow-left'></i></button><button type='button'><i class='block-gallery-next icon-arrow-right'></i></button>";
    ...
    function printSlideIndex() {
      ...
      let position =
        rect.top > 0 ? rect.top + 100 : (window.innerHeight + rect.top) / 2;
      ...
      setTimeout(() => {
        ...
        if (!element) {
          setTimeout(() => {
            position =
              rect.top > 0 ? rect.top : (window.innerHeight + rect.top) / 2;
        ...
        if (element)
          element.closest(".block-carousel-item").classList.add("active");
      }, 250);
    }
  });
  ...
  galleries3.forEach(gallery => {
    ...
    function onInit() {
      ...
      nav.innerHTML =
        "<button type='button'><i class='block-gallery-prev icon-arrow-left'></i></button><button type='button'><i class='block-gallery-next icon-arrow-right'></i></button>";
      counter.innerHTML =
        "<span class='block-gallery-current'>0</span> / <span class='block-gallery-length'>0</span>";
      gallery.parentNode.appendChild(nav);
      ...
      length.innerHTML =
        this.innerElements.length > 10
          ? this.innerElements.length
          : "0" + this.innerElements.length;
...
function navigate(href, delay, delayP = 900, e) {
  ...
  if (!isNav) {
    ...
    if (
      (href.indexOf("./") + 1 && href.length == 2) ||
      (href.indexOf("./") + 1 && href.length == domain.length)
    ) {
    ...
    if (href.indexOf("./") + 1 || href.indexOf(domain) + 1) {
      ...
      if (document.querySelector(".cover-video"))
        document.querySelector(".cover-video").remove();
  ...
  menuToggler.addEventListener("click", () => {
    ...
    if (menuToggler.classList.contains("active")) {
      ...
      isContent
        ...
        : coverAnimCont
        ? coverAnimCont.classList.add("formenu")
        : "";
    } else {
      enable_scroll();
      ...
      setTimeout(() => {
        ...
        let theme = document.elementFromPoint(0, 50).closest("[data-theme]")
          .dataset.theme;
        header.classList.add(theme);
